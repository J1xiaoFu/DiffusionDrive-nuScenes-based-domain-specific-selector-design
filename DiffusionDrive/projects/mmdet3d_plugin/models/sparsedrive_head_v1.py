...
class V1SparseDriveHead(BaseModule):
    ...
    def forward(
        ...
        if self.task_config['with_motion_plan']:
            motion_feature_maps = feature_maps
            if not (
                isinstance(feature_maps, (list, tuple))
                and len(feature_maps) == 3
                and all(torch.is_tensor(x) for x in feature_maps)
                and feature_maps[1].dim() == 3
            ):
                motion_feature_maps = feature_maps_format(feature_maps)
