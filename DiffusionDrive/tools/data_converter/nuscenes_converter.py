...
def empty_map_annos():
    # Labels follow MAP_CLASSES in geom2anno: ped_crossing, divider, boundary.
    return {0: [], 1: [], 2: []}
...
def create_nuscenes_infos(root_path,
                          ...
                          max_sweeps=10,
                          roi_size=(30, 60),
                          allow_missing_map_canbus_for_smoke=False,
    ...
    try:
        ...
        if not allow_missing_map_canbus_for_smoke:
            raise
        print(
            'WARNING: missing or unusable nuScenes map expansion; '
            'writing empty map_annos for smoke-only info generation. '
    ...
    try:
        ...
        if not allow_missing_map_canbus_for_smoke:
            raise
        print(
            'WARNING: missing or unusable nuScenes CAN bus expansion; '
            'writing zero ego_status for smoke-only info generation. '
            f'Original error: {exc}')
        nusc_can_bus = None
    from nuscenes.utils import splits
    available_vers = ['v1.0-trainval', 'v1.0-test', 'v1.0-mini']
    assert version in available_vers
    custom_scene_split = train_scene_names is not None or val_scene_names is not None
    if custom_scene_split:
        train_scenes = list(train_scene_names or [])
        val_scenes = list(val_scene_names or [])
    ...
    metadata = dict(
        version=version,
        allow_missing_map_canbus_for_smoke=allow_missing_map_canbus_for_smoke,
        custom_scene_split=custom_scene_split,
        train_scene_names=list(train_scene_names or []) if custom_scene_split else None,
        val_scene_names=list(val_scene_names or []) if custom_scene_split else None,
    )
    if test:
...
def _fill_trainval_infos(nusc,
    ...
    selected_scenes = set(train_scenes).union(set(val_scenes))
    for sample in mmcv.track_iter_progress(nusc.sample):
        if sample['scene_token'] not in selected_scenes:
            continue
        ...
        if nusc_map_extractor is None:
            map_annos = empty_map_annos()
        else:
...
def parse_args():
    ...
    parser.add_argument(
        '--allow-missing-map-canbus-for-smoke',
        action='store_true',
        help='Allow missing official map/CAN bus expansion packs by writing empty '
             'map annotations and zero ego status. This is only for pipeline smoke '
             'tests and must not be used for experimental validation.')
...
def main():
    ...
    if args.dataset == 'nuscenes' and args.version != 'v1.0-mini':
        ...
        raise ValueError(f'Unsupported dataset/version: {args.dataset} {args.version}')
